...
def id_separator(structure_name):
    """
...
def check_vectors(lig_vector, structure_name, steps, coeff, name_lig, threshold, chain_ids):
    """
...
def coord_lig_atoms(structure, lig_id):
    """
    Функция, находящая все координаты атомов для лиганда
    переменные: structure - структура,  lig_id - id лиганд
    """
    lig_coor =[]
    rec_coor =[]
...
def make_good_vectors(vectors, structure_name, chains_ids):
    ...
    for vector in vectors:
        ...
        check_result_pos = check_vectors(vector, structure_name, 5, 1, 'LIG', 1.75, chains_ids)
        check_result_neg = check_vectors(vector, structure_name, 5, -1, 'LIG', 1.75, chains_ids)
...
def main():
    ...
    structure = parser.get_structure(structure_name, structure_name)
    chains_ids, chains_com = coord_lig_atoms(structure)
    vectors = ligand_vectors(1, chains_com)
    print(len(vectors))
